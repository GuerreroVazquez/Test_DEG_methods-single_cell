...
```{r}
mvso <- makeContrasts(conditions1 - conditions2, levels = colnames(coef(fit_pos)))
yvsm <- makeContrasts(conditions3 - conditions1, levels = colnames(coef(fit_pos)))
yvso <- makeContrasts(conditions3 - conditions2, levels = colnames(coef(fit_pos)))
...
```{r}
tmp <- contrasts.fit(fit_pos, mvso)
tmp <- eBayes(tmp)
```
...
write.csv(top.table, "limma_combat_microarray_pos.csv")
```
...
ggplot(top.table_nF, aes(x = logFC, y = -log10(adj.P.Val))) +
  ...
  theme_minimal() +
  labs(x = "log2(Fold Change)", y = "log10 adj.P.Val", color = "Significant")
