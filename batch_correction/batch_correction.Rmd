...
cts[is.na(cts)] <- 0
cts[cts < 0] <- 0
adjusted <- ComBat(cts, batch=batches)  # change thso to combat vanilla
```
...
# Assuming 'd0' is your DGEList object
d0 <- calcNormFactors(cts)
...
```{r}
ctsx[ctsx < 0] <- 0
adjusted <- ComBat(ctsx, batch=batches)
...
d02 <- DGEList(adjusted)
d02 <- calcNormFactors(d02)
...
```{r}
mm <- model.matrix(~ 0 + conditions + batches, data=exp_design)
y_pre <- voom(d0, mm, plot = T)
y_pos <- voom(d02, mm, plot = T)
```
...
```{r}
fit_pre <- lmFit(y_pre, mm)
fit_pos <- lmFit(y_pos, mm)
head(coef(fit_pre))
head(coef(fit_pos))
```
...
```{r}
fit = fit_pos
title = "limma_combat_microarray_pos.csv"

mvso <- makeContrasts(conditions1 - conditions2, levels = colnames(coef(fit)))
yvsm <- makeContrasts(conditions3 - conditions1, levels = colnames(coef(fit)))
yvso <- makeContrasts(conditions3 - conditions2, levels = colnames(coef(fit)))
...
ggplot(top.table_nF, aes(x = logFC, y = -log10(adj.P.Val))) +
  geom_point(aes(color = ifelse(abs(logFC) >= 1 & (adj.P.Val) < 0.05, "NS", "S"))) +
  theme_minimal() +
  labs(x = "log2(Fold Change)", y = "log10 adj.P.Val", color = "Significant", title = "Pre adjustment Volcano plot")
...
ggplot(top.table_nF, aes(x = logFC, y = -log10(adj.P.Val))) +
  geom_point(aes(color = ifelse(abs(logFC) >= 1 & (adj.P.Val) < 0.05, "NS", "S"))) +
  theme_minimal() +
  labs(x = "log2(Fold Change)", y = "log10 adj.P.Val", color = "Significant", title = "Pos adjustment Volcano plot")
